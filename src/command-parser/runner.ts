...
async function* run_command_v2(msg: Message, cmd: string, cmdObject: CommandV2, args: ArgList, raw_args: ArgumentList, opts: Opts, runtime_options: RuntimeOptions, symbols: SymbolTable, stdin?: CommandReturn, sendCallback?: ((options: MessageCreateOptions | MessagePayload | string) => Promise<Message>), pid_label?: string) {
    ...
    let obj: CommandV2RunArg = {
        ...
        recursionCount: runtime_options.get(
            "recursion",
            runtime_options.get("recursion_limit", RECURSION_LIMIT) - 1
        ),
    ...
    if (cmdObject.argShape) {
        ...
        for await (const [result, type, optional, default_] of cmdObject.argShape(args, msg)) {
            if (result === BADVALUE && !optional) {
                return {
                    content: `Expected ${type}\nUsage: ${generateCommandSummary(cmd, cmdObject)}`,
                    status: StatusCode.ERR
                }
            }
            else if (result === BADVALUE && default_ !== undefined)
                argShapeResults[type] = default_
            else argShapeResults[type] = result
        }
    }
    let runObj = cmdObject.run.bind([cmd, cmdObject])(obj) ?? {
        content: `${cmd} happened`,
...
async function* command_runner(
    ...
    if (disabled && (
        disabled.commands?.includes(cmd) || disabled.categories?.includes(cmdObject?.category)
    )) {
    ...
    let [opts, parsed_args] = opts_parser(
        raw_args,
        (cmdObject as CommandV2).short_opts || "",
        (cmdObject as CommandV2).long_opts || []
    )
    ...
    if (cmdObject instanceof AliasV2) {
        ...
        let rv
        for await (let item of run_command_v2(
            msg,
            cmd,
            cmdObject,
            args,
