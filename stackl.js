...
const { vars, userVars } = require("./common.js");
async function parseArg(arg, argNo, argCount, args, argc, stack, initialArgs, ram, currScopes, msg, recursionC, stacks, SPAMS) {
    switch (arg) {
        //vars
        case "$stacksize": {
        ...
        case "%reply": {
            ...
            if (!(msgToRTo instanceof discord_js_1.Message)) {
        ...
        //embeds
        case "%embed": {
            stack.push(new discord_js_1.MessageEmbed());
            break;
        }
        case "%etitle": {
            let title = stack.pop();
            ...
            let e = stack.pop();
            if (!(e instanceof discord_js_1.MessageEmbed)) {
        ...
        case "%loop": {
            ...
            forever: while (true) {
                ...
                if (topOfStack instanceof discord_js_1.Message) {
...
async function parse(args, useStart, msg, SPAMS) {
    let stacks = { __main__: [] };
    ...
    let ram = {
        ...
        "random": async (low, high) => { low ??= 1; high ??= 10; return Math.random() * (high - low) + low; },
        "input": async (prompt, useFilter, reqTimeout) => {
            ...
            if (useFilter === false || useFilter === 0) {
                ...
                filter = (m) => m.author.id === useFilter && !m.author.bot;
            }
            let timeout = 30000;
